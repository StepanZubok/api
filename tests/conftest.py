...
def authorized_client(client, token, session):
    # Override the get_current_user_id dependency to use the token directly
    def override_get_current_user():
        from jose import jwt
        from app.config import settings
        payload = jwt.decode(token, settings.SECRET_KEY, algorithms=[settings.ALGORITHM])
        user_id = payload.get("user_id")
        user = session.query(models.UsersTable).filter(models.UsersTable.id == user_id).first()
        return user
...
def test_posts(test_user, session, test_user_2):
    posts_data = [
        {"title": "a", "text": "B", "account_id": test_user["id"]}, 
        {"title": "b", "text": "c", "account_id": test_user["id"]}, 
        {"title": "b", "text": "c", "account_id": test_user_2["id"]}
    ]
